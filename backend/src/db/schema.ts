...
export const sessions = pgTable('sessions', {
  ...
  summaryShared: jsonb('summary_shared').$type<any[]>().notNull().default([]),
  summaryTomorrow: jsonb('summary_tomorrow').$type<any[]>().notNull().default([]),
  summaryLetGo: jsonb('summary_let_go').$type<any[]>().notNull().default([]),
  createdAt: timestamp('created_at').defaultNow().notNull(),
});
